...
@section('content')
    <main>
        ...
        <div>
            ...
            {{-- <div class="p-2">
                ...
                @if (count($similar_items_x_photos))
                <div class="mt-2">
                    <x-photo-sugestions :similaritemsxphotos="$similar_items_x_photos" resultid="div-search-result"></x-photo-sugestions>
                </div>
                @else
                <div class="text-slate-500 text-sm italic">- Belum ada saran photo dari barang lain -</div>                    
                @endif
                <div id="result-similar-items" class="relative w-full"></div>
            </div> --}}

            <div class="p-2">
                <div class="inline-block bg-white shadow drop-shadow p-1">
                    <h2 class="text-slate-500 font-bold">Pilih Photo</h2>
                </div>
            </div>
                ...
                @for ($i = 0; $i < 5; $i++)
                    @if ($item_photos[$i])
                        <div class="flex gap-2">
                            ...
                            <form
                                action="{{ route('items.delete_photo', [$item->id, $item_photos[$i]->id, $photos[$i]->id]) }}"
                                method="POST" onsubmit="return confirm('Anda yakin ingin hapus photo_item ini?')"
                    ...
                    @else
                        <div>
                            <a href="{{ route('items.pilihan_photo', [$item->id, $i]) }}">
                                <label for="input-photo-{{ $i }}" class="inline-block hover:cursor-pointer"
                                    id="label-input-photo-{{ $i }}">
                        ...
                        {{-- <form method="POST" action="{{ route('items.add_photo', $item->id) }}" class="mb-1"
                            ...
                            <input id="input-photo-{{ $i }}" type="file" name="photo"
                                onchange="previewImage(this.files[0], 'div-preview-photo-{{ $i }}', 'preview-photo-{{ $i }}', 'label-input-photo-{{ $i }}')"
                                class="hidden">
                            ...
                            <div id="div-preview-photo-{{ $i }}" class="hidden">
                                <div class="flex justify-end">
                                    <button type="button" class="text-red-400"
                                        onclick="removeImage('input-photo-{{ $i }}', 'div-preview-photo-{{ $i }}', 'preview-photo-{{ $i }}', 'label-input-photo-{{ $i }}')">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <script>
        // setTimeout(() => {
        //     console.log(similar_items_x_photos);
        // }, 1000);
    </script>
@endsection
