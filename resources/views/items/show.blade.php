...
@section('content')
            ...
            <div id="default-carousel" class="relative w-full" data-carousel="static">
                ...
                <div class="absolute z-20 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
                    @foreach ($photos_sorted as $key => $photo)
                        ...
                        @else
                            <button type="button" class="w-3 h-3 rounded-full bg-white/50 dark:bg-gray-800/50"
                                aria-current="false" aria-label="Slide {{ $key }}"
                                data-carousel-slide-to="{{ $key }}"></button>
                        @endif
                    @endforeach
                </div>
                        ...
                        @if (count($item->item_matas) > 0)
                            <div class="flex gap-2">
                                @foreach ($item->item_matas as $item_mata)
                                    ...
                                    <span
                                        class="italic text-slate-400">({{ $item_mata->mata->level_warna }}-{{ $item_mata->mata->opacity }})</span>
                                @endforeach
                            </div>
                        @else
                            <div>-</div>
    ...
    <script>
        const user_id = {{ json_encode($user->id, JSON_HEX_TAG) }}
        ...
        getScrollLocation(`${user_id}-items.show`);
    </script>
@endsection
