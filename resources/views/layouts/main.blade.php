            ...
            <div class="flex h-16 items-center justify-between">
              ...
              <div class="">
                <div class="ml-4 flex items-center md:ml-6 gap-2">
                    @auth
                    <a href="{{ route('add_new_item.pilih_tipe_barang', 'items') }}" class="w-7 h-7 rounded-full bg-emerald-300 flex justify-center items-center font-bold text-white text-2xl">+</a>
                    @endauth
                  ...
                  <a class="loading-spinner" href="{{ route('carts.index', Auth::user()->id) }}">
                    <div class="text-indigo-500 relative">
                        ...
                        @if ($cart)
                        <div class="flex absolute left-3 -top-3 w-5 h-5 rounded-full bg-red-400 text-white justify-center items-center">{{ count($cart->cart_items) }}</div>
                        @endif
                    </div>
                  </a>
                  @endif
        ...
        <div class="mt-16 mx-2 mb-16 p-2 rounded border-2 border-emerald-300 text-gray-400 no-print">
            ...
            <form action="{{ route('logout') }}" method="POST">
                ...
                <div class="flex justify-center">
                    <button type="submit" class="loading-spinner relative z-10 rounded-md border-2 border-yellow-400 bg-yellow-200 py-1 px-2 font-medium hover:bg-gray-700 hover:text-white flex gap-1 items-center">
                        <span>Log out</span>
