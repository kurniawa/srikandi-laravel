...
<main class="p-2">
        ...
        <form action="{{ route('attributes.caps.store') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-1">
                <div class="col-span-1">
                    <input type="text" name="nama" id="nama" placeholder="nama cap" class="rounded w-full">
                </div>
                <div class="col-span-1">
                    <input type="text" name="codename" id="codename" placeholder="codename cap" class="rounded w-full">
                </div>
            </div>
            <div class="flex justify-center mt-2">
                <div class="rounded p-2 bg-emerald-300 text-white font-bold">
                    <button type="submit">Konfirmasi +Tambah Cap</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="border-2 border-orange-300 rounded p-1 mt-5">
        <div class="text-center mt-2">
            <h3 class="font-bold text-slate-500">Daftar Cap</h3>
        </div>
        <div class="mt-2">
            <table class="table-slim w-full">
                ...
                @foreach ($caps as $cap)
                <tr>
                    <td class="text-center">{{ $cap->nama }}</td>
                    <td class="text-center">{{ $cap->codename }}</td>
                    <td>
                        <div class="flex gap-1">
                            <a href="{{ route('attributes.caps.edit', $cap->id) }}">
                            ...
                            <form action="{{ route('attributes.caps.destroy', $cap->id) }}" method="POST" onsubmit="return confirm('Yakin ingin hapus cap ini?')">
                                @csrf
