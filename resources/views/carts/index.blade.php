...
<main class="p-2">
    ...
    <form action="{{ route('carts.checkout', $cart->id) }}" method="GET" class="mt-5">
        ...
        <div class="grid grid-cols-12 border-y py-3 gap-1">
            ...
            <div class="col-span-3 foto-barang">
                <div class="flex h-full items-center">
                    ...
                    <div class="w-full">
                        <img src="{{ asset("storage/" . $cart_item->item->photos[0]->path) }}" alt="item_photo" class="w-full">
                    </div>
                    @else
            ...
            <div class="col-span-9">
                <a href="{{ route('items.show', [$cart_item->item->id, 'carts.index']) }}" class="font-bold text-indigo-500">{{ $cart_item->item->nama_short }}</a>
            ...
            <div class="col-span-12 mt-2">
                <div class="flex justify-between">
                    <div>
                        <a href="{{ route('cart_items.add_photo', [$cart->id, $cart_item->id]) }}" class="border-2 border-emerald-300 text-emerald-300 font-bold p-1 rounded text-xs">Foto Cart Item</a>
                    </div>
                    <div class="flex justify-end">
                        <div class="flex">
                            <label for="btn-delete-cart-item-{{ $key }}" class="loading-spinner border-y border-l rounded-l-xl w-6 h-6 flex items-center justify-center text-slate-500 hover:cursor-pointer">
        ...
        <div class="flex justify-center mt-9">
            <button type="submit" class="rounded-lg px-3 py-2 bg-emerald-300 text-white border-2 border-emerald-400 font-bold">Checkout</button>
        </div>
    </form>
    @else
    ...
    @if ($cart)
    @foreach ($cart->cart_items as $key => $cart_item)
    <form action="{{ route('cart_items.delete', [$cart->id, $cart_item->id]) }}" method="POST" onsubmit="return confirm('Ingin hapus item ini dari keranjang?')">
        @csrf
