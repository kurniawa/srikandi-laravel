...
<main class="p-2">
    ...
    <div class="">
        ...
        <div id="tombol-toggle" class="mt-2">
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" value="" class="sr-only peer" onclick="toggleBuyback({{ $key }}, this.checked)">
        ...
        <div class="grid-cols-12 bg-violet-100 text-slate-500 p-2 rounded">
            ...
            <table class="mt-2">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="berat_buyback_formatted" id="berat_buyback_formatted" class="rounded p-1" value="{{ old('berat_buyback_formatted') }}">
                        <input type="hidden" id="berat_buyback" name="berat_buyback" value="{{ old('berat_buyback') }}">
                    </td>
                    @else
                    <td>
                        <input type="text" name="berat_buyback_formatted" id="berat_buyback_formatted" class="rounded p-1" value="{{ casual_decimal_format($surat_pembelian_item->berat) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="potongan_ongkos_formatted" id="potongan_ongkos_formatted-{{ $key }}" class="rounded p-1" onchange="formatNumber(this, 'potongan_ongkos-{{ $key }}');hitungHargaBuyback({{ $key }})" value="{{ old('potongan_ongkos_formatted') }}">
                        <input type="hidden" name="potongan_ongkos" id="potongan_ongkos-{{ $key }}"  value="{{ old('potongan_ongkos') }}">
                    </td>
                    @else
                    <td>
                        <input type="text" name="potongan_ongkos_formatted" id="potongan_ongkos_formatted-{{ $key }}" class="rounded p-1" onchange="formatNumber(this, 'potongan_ongkos-{{ $key }}');hitungHargaBuyback({{ $key }})" value="{{ casual_decimal_format((float)$surat_pembelian_item->ongkos_g * (float)$surat_pembelian_item->berat / 100) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="potongan_mata_formatted" id="potongan_mata_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_mata-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1" value="{{ old('potongan_mata_formatted') }}">
                        <input type="hidden" name="potongan_mata" id="potongan_mata-{{ $key }}" >
                    </td>
                    @else
                    <td>
                        <input type="text" name="potongan_mata_formatted" id="potongan_mata_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_mata-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1">
                        <input type="hidden" name="potongan_mata" id="potongan_mata-{{ $key }}">
                    </td>
                    @endif
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="potongan_rusak_formatted" id="potongan_rusak_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_rusak-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1" value="{{ old('potongan_rusak_formatted') }}">
                        <input type="hidden" name="potongan_rusak" id="potongan_rusak-{{ $key }}" value="{{ old('potongan_rusak') }}">
                    </td>
                    @else
                    <td>
                        <input type="text" name="potongan_rusak_formatted" id="potongan_rusak_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_rusak-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="potongan_susut_formatted" id="potongan_susut_formatted-{{ $key }}" class="rounded p-1" onchange="formatNumber(this, 'potongan_susut-{{ $key }}');hitungHargaBuyback({{ $key }})" value="{{ old('potongan_susut_formatted') }}">
                        <input type="hidden" name="potongan_susut" id="potongan_susut-{{ $key }}"  value="{{ old('potongan_susut') }}">
                    </td>
                    @else
                    <td>
                        <input type="text" name="potongan_susut_formatted" id="potongan_susut_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_susut-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="potongan_lain_formatted" id="potongan_lain_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_lain-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1" value="{{ old('potongan_lain_formatted') }}">
                        <input type="hidden" name="potongan_lain" id="potongan_lain-{{ $key }}"  value="{{ old('potongan_lain') }}">
                    </td>
                    @else
                    <td>
                        <input type="text" name="potongan_lain_formatted" id="potongan_lain_formatted-{{ $key }}" onchange="formatNumber(this, 'potongan_lain-{{ $key }}');hitungHargaBuyback({{ $key }})" class="rounded p-1">
                ...
                <tr>
                    ...
                    <td>
                        <textarea name="keterangan_buyback" id="keterangan_buyback" rows="3" class="border rounded p-2">{{ old('keterangan_buyback') }}</textarea>
                    </td>
                    @else
                    <td>
                        <textarea name="keterangan_buyback" id="keterangan_buyback" rows="3" class="border rounded p-2"></textarea>
                    </td>
                    @endif
                </tr>
            </table>
            <div class="flex justify-end">
                <div>
                    ...
                    <div class="font-bold text-rose-400 text-xl"><span>Rp</span> <span id="harga_buyback-{{ $key }}"></span></div>
                </div>
            </div>
        </div>
        @endforeach
...
<script>
    ...
    function hitungHargaBuyback(index) {
        ...
        document.getElementById(`harga_buyback-${index}`).textContent = formatNumberX(preformatDotToComa(pangkasDesimal(harga_buyback)));
    }
