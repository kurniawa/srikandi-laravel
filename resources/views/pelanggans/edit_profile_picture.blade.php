        ...
        <div>
            @if ($pelanggan->profile_picture_path)
                ...
                <form action="{{ route('pelanggans.delete_profile_picture', $pelanggan->id) }}" method="POST"
                    onsubmit="return confirm('Anda yakin ingin hapus foto pelanggan ini?')" class="mt-3">
                    @csrf
            ...
            @else
                <form method="POST" action="{{ route('pelanggans.update_profile_picture', $pelanggan->id) }}"
                    class="mb-1" enctype="multipart/form-data">
                    @csrf
                    {{-- PROFILE PICTURE --}}
                    <!-- Foto Profile -->
                    ...
                    <div id="label-input-profile-photo" class="flex justify-center">
                        <label for="input-profile-photo"
                            class="border-2 border-rose-300 rounded p-2 mt-2 hover:cursor-pointer">
